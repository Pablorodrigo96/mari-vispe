...
export function MatchAnalyticsContent() {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <SideTable
          ...
          rows={mandList.map((m: any) => ({
            id: m.id, kind: "mandate",
            primary: m.company_cnpj,
            secondary: `${m.uf || "—"} · ${m.setor || "—"}`,
            contact: m.contato_nome, phone: m.contato_telefone, email: m.contato_email,
          }))}
        />
        <SideTable
...
function SideTable({
  ...
  return (
    <div className="bg-zinc-900/60 border border-zinc-800 rounded-lg backdrop-blur-md overflow-hidden">
      <div className={cn("px-4 py-2.5 border-b text-xs font-semibold uppercase tracking-wider", accent)}>{title}</div>
      <div className="max-h-[480px] overflow-y-auto divide-y divide-zinc-800">
        {rows.length === 0 && <div className="text-xs text-zinc-500 p-4">Nenhum registro.</div>}
        {rows.map((r) => (
          <Link key={r.id} to={`/equity-brain/crm/${r.kind}/${r.id}`} className="block px-4 py-2.5 hover:bg-zinc-900/80 transition">
