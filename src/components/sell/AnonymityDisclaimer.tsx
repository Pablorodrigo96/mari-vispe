import { ShieldCheck, EyeOff, FileSignature } from 'lucide-react';
    ...
    return (
      <div className="flex items-start gap-2 rounded-md border border-accent/30 bg-accent/5 p-3 text-xs text-foreground/90">
        ...
        <p className="break-words">
          <span className="font-semibold">Anonimato garantido:</span> sua empresa aparece apenas com codinome
          (ex: <code className="text-accent">MARI-XXX-1234</code>). Razão social, CNPJ e endereço completo só são revelados após NDA assinado.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-accent/30 bg-gradient-to-br from-accent/5 to-transparent p-5 mb-6">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-semibold text-foreground">Sua identidade é protegida</h3>
      </div>
      <ul className="space-y-2 text-sm text-foreground/85">
        <li className="flex items-start gap-2 break-words">
          <EyeOff className="h-4 w-4 text-accent shrink-0 mt-0.5" />
          <span>Sua empresa entra na vitrine apenas com <span className="font-semibold">codinome</span> (ex: <code className="text-accent">MARI-TECH-1234</code>). Razão social, CNPJ, endereço completo e fotos identificáveis <span className="font-semibold">não aparecem</span> publicamente.</span>
        </li>
        <li className="flex items-start gap-2 break-words">
