...
export function ReservationModal({ open, onClose, token }: { open: boolean; onClose: () => void; token: any }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="!bg-carbon border-bone/10 text-bone max-w-md p-0 overflow-hidden gap-0">
        {/* HEADER simplificado */}
        <div className="px-6 pt-6 pb-4 border-b border-bone/10">
          <div className="text-[11px] uppercase tracking-wider text-bone/40">Você está investindo em</div>
          <div className="text-lg font-semibold text-bone mt-0.5 break-words leading-tight">
        ...
        {status === "ready" && step === 1 && (
          <div className="p-6 space-y-5">
            ...
            <div className="flex items-center justify-between bg-graphite/50 border border-bone/10 rounded-xl px-4 py-3">
              ...
              <button
                type="button"
                onClick={() => { onClose(); navigate("/investir/carteira"); }}
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-bone/50 font-semibold">R$</span>
                <input
                  type="text"
                  inputMode="decimal"
                  ...
                  className="w-full bg-graphite/40 border border-bone/10 rounded-xl py-4 pl-12 pr-4 text-2xl font-semibold text-bone tabular-nums focus:border-volt/50 focus:outline-none"
                />
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {[100, 500, 1000].map(v => (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs bg-bone/5 hover:bg-volt/15 hover:text-volt border border-bone/10 rounded-full text-bone/70 transition-colors"
                  >
                    +{fmtBRL(v)}
                  </button>
                ))}
            ...
            <div className="bg-volt/5 border border-volt/20 rounded-xl p-4">
              <div className="text-sm text-bone/70">Você recebe</div>
              <div className="text-2xl font-semibold text-bone tabular-nums mt-1">
                {qty.toLocaleString("pt-BR", { maximumFractionDigits: 4 })} <span className="text-base text-bone/60 font-normal">cotas</span>
              </div>
            ...
            {belowMin && (
              <div className="flex items-start gap-2 text-xs text-amber-300 bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg">
                ...
                O mínimo desta oferta é {fmtBRL(minTicket)}.
              </div>
            )}
            {insufficient && (
              <div className="flex items-start gap-2 text-xs text-amber-300 bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg">
                <AlertCircle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                Saldo insuficiente.{" "}
                <button onClick={() => { onClose(); navigate("/investir/carteira"); }} className="underline font-medium">
                  Adicionar saldo
                </button>
              </div>
            )}
        ...
        {status === "ready" && step === 2 && (
          <div className="p-6 space-y-5">
            <button
              onClick={() => setStep(1)}
              className="text-xs text-bone/50 hover:text-bone flex items-center gap-1"
            >
            ...
            <label className="flex items-start gap-3 text-xs text-bone/65 cursor-pointer">
              ...
              />
              <span>
                Li e concordo com os <a href="#" className="text-volt hover:underline">documentos da oferta</a>{" "}
                e entendo que investimentos em empresas privadas envolvem risco de perda.
              </span>
            </label>
