...
BEGIN
  ...
  SELECT
    a.cnpj, 'socio_idade_max', a.max_age::numeric,
    CASE WHEN a.max_age >= 65 THEN 1.0
         WHEN a.max_age >= 55 THEN 0.7
         WHEN a.max_age >= 45 THEN 0.3 ELSE 0.1 END,
    'receita_federal', 0.95,
    LEAST(1.0, GREATEST(0.0, (a.max_age - 35)::numeric / 35.0)),
    'observed', NOW(), 3650, NOW()
  FROM age_per_company a
  ON CONFLICT (cnpj, signal_key) DO UPDATE
    SET signal_value = EXCLUDED.signal_value, weight = EXCLUDED.weight,
        confidence = EXCLUDED.confidence, p_true = EXCLUDED.p_true,
        evidence_ts = EXCLUDED.evidence_ts, updated_at = NOW();
  GET DIAGNOSTICS v_signals = ROW_COUNT;

  INSERT INTO equity_brain.company_signals
    (cnpj, signal_key, signal_value, weight, source, confidence,
     p_true, evidence_strength, evidence_ts, freshness_decay_days, updated_at)
  SELECT c.cnpj, 'tempo_atividade_anos',
    EXTRACT(YEAR FROM AGE(c.data_abertura))::numeric,
    1.0, 'receita_federal', 1.0,
    LEAST(1.0, EXTRACT(YEAR FROM AGE(c.data_abertura))::numeric / 30.0),
    'observed', NOW(), 365, NOW()
  FROM equity_brain.companies c
  WHERE c.data_abertura IS NOT NULL
  ON CONFLICT (cnpj, signal_key) DO UPDATE
    SET signal_value = EXCLUDED.signal_value, p_true = EXCLUDED.p_true,
        evidence_ts = EXCLUDED.evidence_ts, updated_at = NOW();
  ...
  INSERT INTO equity_brain.company_signals
    (cnpj, signal_key, signal_value, weight, source, confidence,
     p_true, evidence_strength, evidence_ts, freshness_decay_days, updated_at)
  SELECT c.cnpj, 'sweet_spot_fadiga',
  ...
  WITH parts AS (
    SELECT cnpj, signal_key, signal_value FROM equity_brain.company_signals
  ...
  scored AS (
    SELECT cnpj, LEAST(1.0,
        0.35 * CASE WHEN idade IS NULL THEN 0
                    WHEN idade >= 65 THEN 1.0
                    WHEN idade >= 55 THEN (idade - 55) / 10.0 * 0.7 + 0.3 ELSE 0 END
      + 0.25 * CASE WHEN tempo IS NULL THEN 0
                    WHEN tempo BETWEEN 8 AND 20 THEN 1.0
                    WHEN tempo BETWEEN 5 AND 7 THEN 0.5
                    WHEN tempo > 20 THEN 0.6 ELSE 0 END
      + 0.25 * COALESCE(unip, 0)
      + 0.15 * COALESCE(sweet, 0)) AS score
    FROM pivoted
  )
  INSERT INTO equity_brain.company_signals
    (cnpj, signal_key, signal_value, weight, source, confidence,
  ...
  SELECT cnpj, 'seller_intent_score', score, 1.0, 'derived',
         0.85, score, 'inferred', NOW(), 180, NOW()
  FROM scored
  ON CONFLICT (cnpj, signal_key) DO UPDATE
    SET signal_value = EXCLUDED.signal_value, p_true = EXCLUDED.p_true,
        evidence_ts = EXCLUDED.evidence_ts, updated_at = NOW();
