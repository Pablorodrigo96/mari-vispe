CREATE OR REPLACE FUNCTION public.fn_deal_doc_signed_notify()
 ...
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'net', 'equity_brain'
AS $function$
DECLARE
...
BEGIN
  IF NEW.status <> 'signed' OR COALESCE(OLD.status,'') = 'signed' THEN
    RETURN NEW;
  END IF;
  IF NEW.deal_pair_id IS NULL THEN RETURN NEW; END IF;
  ...
  SELECT m.company_cnpj, COALESCE(m.created_by, m.responsavel_id)
    ...
    FROM equity_brain.mandates m WHERE m.id = v_pair.sell_mandate_id LIMIT 1;

  IF v_company_cnpj IS NOT NULL THEN
    SELECT c.codename INTO v_codename FROM equity_brain.companies c WHERE c.cnpj = v_company_cnpj LIMIT 1;
  END IF;
  v_codename := COALESCE(v_codename, 'Projeto');

  IF v_pair.buyer_profile_id IS NOT NULL THEN v_buyer_user := v_pair.buyer_profile_id; END IF;
  ...
  IF v_is_nbo AND v_pair.status IN ('active','draft','nbo') THEN
    UPDATE public.deal_pairs SET status='signed', nbo_signed_at=now(), updated_at=now() WHERE id=v_pair.id;
  ELSIF v_is_closing AND v_pair.status IN ('signed','active','nbo') THEN
    UPDATE public.deal_pairs SET status='closed', closed_at=now(), updated_at=now() WHERE id=v_pair.id;
  END IF;
