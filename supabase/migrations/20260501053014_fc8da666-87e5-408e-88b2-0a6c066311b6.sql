CREATE OR REPLACE FUNCTION public.eb_today_cards(p_limit integer DEFAULT 7)
 RETURNS TABLE(card_kind text, ref_id uuid, mandate_id uuid, mandate_codename text, mandate_value numeric, priority_score integer, match_score numeric, contact_id uuid, contact_name text, contact_phone text, days_inactive integer, headline text, subline text, computed_at timestamp with time zone)
 LANGUAGE plpgsql
