-- Fix eb_today_cards: m.codename não existe em equity_brain.mandates
-- (vive em equity_brain.companies). JOIN para resolver.
...
BEGIN
  ...
  WITH cooling AS (
    ...
    WHERE (m.responsavel_id = v_user
      ...
      AND COALESCE(m.pipeline_stage, '') NOT IN ('closed','won','lost','cancelled','archived')
