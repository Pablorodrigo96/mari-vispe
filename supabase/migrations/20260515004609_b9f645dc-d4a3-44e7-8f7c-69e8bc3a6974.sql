...
DECLARE
  ...
  v_full_name TEXT;
  admin_rec RECORD;
  -- Lista efetiva de "roles" derivada de `profile` (string) ou `roles` (array legado).
...
BEGIN
  ...
  -- Profile (sempre cria; flag is_partner_accountant aplicada abaixo se perfil = partner)
  ...
  -- Mapeia o `profile` (Bloco 2 / radio único) para roles efetivas.
  -- partner = comprador + flag de parceiro contábil (auto-grant; sem approval).
  IF meta_profile IS NOT NULL THEN
    ...
    END IF;
  -- Fallback: formato legado `roles` (array). Mantém compat com signups antigos/edge cases.
