// Equity Brain — calculate-sav-score-batch
// Wrapper admin-only para rodar SAV em batch com filtro reforçado (Pablo).
// Filtro: is_current=true AND abstain=false AND sav_score IS NULL
//         AND match_score>=60 AND setor_fit>=0.7
// Ordenação: match_score DESC, tese_fit DESC NULLS LAST
// Paralelismo: grupos de 10. Guard: >20% erro nos primeiros 50 → abortar.
...
async function calcOne(supabase: any, matchId: string) {
  ...
  const sav = Math.round(
    sinergiaCusto * 0.25 + sinergiaReceita * 0.15 + eliminacao * 0.15 +
    geografia * 0.15 + custoOportunidade * 0.15 + encaixe * 0.15
  );

  const breakdown = {
    sinergia_custo: sinergiaCusto, sinergia_receita: sinergiaReceita,
    eliminacao_competitiva: eliminacao, geografia, custo_oportunidade: custoOportunidade,
    encaixe_estrategico: encaixe, historical_deals_count: historicalDeals.length, buyer_tipo: tipo,
  };

  const { error: upErr } = await supabase.schema("equity_brain" as any)
    .from("matches").update({
      sav_score: sav, sav_breakdown: breakdown, sav_calculated_at: new Date().toISOString(),
    }).eq("id", matchId);
...
serve(withObservability(async (req) => {
  ...
  try {
    ...
    const limit = Math.min(Number(body.limit ?? 880), 2000);
    const concurrency = Math.min(Number(body.concurrency ?? 10), 20);
    ...
    for (let i = 0; i < ids.length; i += concurrency) {
      ...
      const results = await Promise.allSettled(slice.map((id) => calcOne(supabase, id)));
      for (const r of results) {
        processed++;
        if (r.status === "fulfilled") success++;
        else { errors++; if (errSamples.length < 5) errSamples.push(String(r.reason).slice(0, 200)); }
      }
      if (processed >= 50 && !aborted && errors / processed > 0.20) {
        aborted = true;
        console.error(`SAV batch aborted at ${processed}: ${errors} errors (>20%)`);
        break;
      }
      if (processed % 200 < concurrency) {
        console.log(`SAV batch progress: ${processed}/${total} (success=${success}, errors=${errors})`);
      }
    }

    return new Response(JSON.stringify({
      total_candidates: total, processed, success, errors,
      aborted, error_samples: errSamples,
    }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    console.error("calculate-sav-score-batch error:", e);
    return new Response(JSON.stringify({ error: e instanceof Error ? e.message : "Unknown" }), {
      status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
}, { name: "calculate-sav-score-batch" }));
